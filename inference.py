...
def main():
    ...
    model = ReflectionRemovalModel(config)
    ...
    with torch.no_grad():
        ...
        # Save initial noise
        save_image(x, os.path.join(args.output_dir, 'timestep_1000_noise.jpg'))
        ...
        for i in tqdm(range(config.TIMESTEPS-1, -1, -1), desc='Sampling'):
            ...
            if i % args.save_interval == 0 or i == config.TIMESTEPS-1 or i == 0:
                save_image(x, os.path.join(args.output_dir, f'timestep_{i:04d}.jpg'))
        
        # Save final result
        save_image(x, os.path.join(args.output_dir, 'final_result.jpg'))
